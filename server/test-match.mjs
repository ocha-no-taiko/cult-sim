...
async function attack(auw, buw, label, expect) {
  a.send({ t: 'sync', underworld: auw })
  b.send({ t: 'sync', underworld: buw })
  await wait(150)
  a.send({ t: 'assassinate', target: started.players.find((p) => p.name === '防御側').id, funds: FUNDS })
  const r = await a.take('attackResult')
  check(label, r?.outcome === expect, `→ ${r?.outcome}${r?.reason ? '（' + r.reason + '）' : ''}`)
  return r
}

await attack(10, 50, '闇度が足りないと差し向けられない', 'unable')
await wait(6200)   // 翌日まで待つ
await attack(50, 10, '相手が裏社会と無縁なら手が出ない', 'unable')
await wait(6200)
await attack(50, 35, '自分の方が濃ければ成功', 'success')

const killed = await b.take('killed')
check('狙われた側に暗殺が通知される', killed?.by === '攻撃側')
await wait(600)
check('暗殺された教団は脱落扱いになる',
  a.room?.players?.find((p) => p.name === '防御側')?.alive === false)
...
console.log(`\n${pass} 件成功 / ${fail} 件失敗`)
for (const c of [a, b, c1, c2, d1, d2, e1, e2]) c.ws.close()
